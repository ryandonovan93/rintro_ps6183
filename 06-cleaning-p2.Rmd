...
2.  Click Environment in the Environment Pane -\> Import Dataset -\> From Text(base) -\> Select **flanker1.csv -\>** change its name to **df_flanker1**
3.  Click Environment in the Environment Pane -\> Import Dataset -\> From Text(base) -\> Select **flanker2.csv -\>** change its name to **df_flanker**
4.  Follow the same instructions to import the **demographics.csv** and **reaction_time.csv** files. Change their name to **df_demographics** and **df_rt** respectively.
5.  [See Chapter 4's section on importing data for more information](#importing)

Alternatively, you can write and run the following commands within the **data_cleaning_ii_script.R**

```{r eval = F}

df_background <- read.csv("datasets/background.csv") 

df_flanker1 <- read.csv("datasets/flanker1.csv")

df_flanker2 <- read.csv("datasets/flanker2.csv")
...
# Change missing values in the age column with mean age
...
[In the last chapter, I asked you to clean a Flanker task](#clean-activity). However, this flanker task was actually composed of separate flanker.csv files. This week, we are going to join two of these two separate files together (see `df_flanker1` and `df_flanker2` from the set up at the start of the chapter). Additionally, we are going to clean the associated demographic file (`df_background`).

First let's clean the `df_background file`, then we will clean the `df_flanker1` and `df_flanker2` files. At the end then we will merge them together.
...
Again, not exactly a data frame to write to your parents about. There is a lot of cleaning we need to do here. Let's go through it step-by-step again.

First thing we need to do is select our columns. Most of the columns here are not necessary, the columns we need are:
...
Okay that's a lot easier. Our data frame is certainly smaller. If you look at the values in the Participant.Private.ID you might notice that there is a lot of repeated values. This is because this data is in long format rather than wide.

We are going to change that in a couple of steps. But the next thing we are going to do is fix our column names using the `rename()` function. Remember that the syntax is: `rename(df, new name = old name)`
...
Through just two functions we have dramatically shortened the size of our data frame. Now we will need to think about cleaning our rows. If you look at the values under `Question` and `Response`, you will see relatively strange responses. Let's talk through them.
...
-   Question - Response
    ...
    -   Age-quantised - Again this translates the participants response to a numerical value in relation to the order options were displayed. Not needed.

    -   Age-text - Again, no clue. But we do not need.

So we only need the rows where the value under the Question column equals Age or Sex. We can do this using the filter() function and the \| operator.
...
Just one last thing we need to do now, which is to pivot our data frame from long to wide using our new friend the `pivot_wider()` function. 

```{r}

df_background_clean <- pivot_wider(df_background_filter,
                                   id_cols = ID,
                                   names_from = Question,
                                   values_from = Response)
...
Perfect, let's move on to cleaning our `df_flanker1` and `df_flanker2` data frames. 
