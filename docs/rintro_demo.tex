...
\begin{figure}
\centering
\includegraphics{rintro_demo_files/figure-latex/unnamed-chunk-8-1.pdf}
...
\begin{figure}
\centering
\includegraphics{rintro_demo_files/figure-latex/unnamed-chunk-9-1.pdf}
...
Here R tells us that the first answer (i.e., value) corresponds to \texttt{0.1}. The next label is \texttt{{[}16{]}}. which tells us that the 16th answer corresponds to 5.5.
...
\caption{\label{fig:unnamed-chunk-51}Indexing for Numeric Vector}
\end{figure}

\begin{figure}
\centering
\includegraphics{img/03-index-character.png}
\caption{\label{fig:unnamed-chunk-52}Indexing for Character Vector}
\end{figure}

\begin{figure}
\centering
\includegraphics{img/03-index-logical.png}
\caption{\label{fig:unnamed-chunk-53}Indexing for Logical Vector}
\end{figure}
...
\caption{\label{fig:unnamed-chunk-67}The relationship between data frames and vectors. The different colours in the data frame indicate they are composed of independent vectors}
\end{figure}
...
The \textbf{\$} Notation: You can use a dollar sign (\$) followed by the column to select \textbf{\emph{an individual column}} in a data frame. For example, let's select the \texttt{Name} column in the \texttt{my\_df} data frame.
...
\textbf{Square Brackets} \texttt{{[}{]}}: This is similar approach to accessing elements from a vector. Inside the brackets you can specify both the row and columns that you want to extract. The syntax for selecting rows and columns is: \texttt{the\ dataframe{[}the\ rows\ we\ want,\ the\ columns\ we\ want{]}}.
...
Rows in a data frame represent individual observations or records. You can access rows using indexing, specifying the row number you want to retrieve, following the syntax: \texttt{the\ dataframe{[}the\ rows\ we\ want,\ the\ columns\ we\ want{]}}.
...
We can also select both rows and columns using \texttt{{[}{]}} and our syntax: \texttt{the\ dataframe{[}the\ rows\ we\ want,\ the\ columns\ we\ want{]}}.

For example we could select the first and third rows for the \texttt{Age} and \texttt{Score} columns.
...
\hypertarget{exercises---data-structures}{%
\subsection{Exercises - Data Structures}\label{exercises---data-structures}}
