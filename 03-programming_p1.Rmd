...
We can command the R console to compute calculations. Remember not to type the operator ´\>´ if you are following along in RStudio, this just indicates that R is ready to execute a new command.[^03-programming_p1-1]
...
Here R tells us that the first answer (i.e., value) corresponds to `0.1`. The next label is [17], which tells us that the 17th answer corresponds to 5.5. The reason that R does this is that R only prints out the index in the console each time it needs to make a new row. This helps us orient ourselves to answers, without adding a lot more data (e.g., if there was an index for every single answer that would double the number of data we were looking at).
...
R thinks you making the claim that 10 equals 20. Since this is not true, R panics and refuses to run your command. Now any person looking at your code would guess that since `+` and `=` are on the same key on our keyboards, you probably meant to type `10 + 20`. But that's because we have a strong theory of mind, whereas programming languages do not.
...
We can use variables compute calculations with their information. Let's say I have several variables relating to my scores on five items measuring Extraversion (which I will label with the variable names `extra1` to `extra5`). I can use the variable names to calculate my total Extraversion score.
...
You can also use square brackets to access columns. Inside the brackets, you specify the row indices (or leave it empty for all rows) and the column name in quotes. For example, to access the "Age" column of my_df:
...
To get the first row of your data frame (my_df):
...
Following on from these examples, we can select both rows and columns using `[]`.For example we could select the first and third rows for the `Age` and `Score` columns.
